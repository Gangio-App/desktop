...
const os = require('os'); // likely already there
const APP_UPDATE_COMPLETE_HTML = path.join(__dirname, 'update-complete.html');

let appReadyIpcHandlersRegistered = false; // Global flag for IPC handlers
...
const APP_LOGIN_URL = 'https://gangio.pro/login';
...
if (app.isPackaged) {
  try {
    const iconPath = path.join(process.resourcesPath, 'resources', 'icons', 'icon.ico');
    if (fs.existsSync(iconPath)) {
      app.setAppUserModelId(APP_ORGANIZATION + '.' + APP_NAME.replace(' ', ''));
    }
  } catch (error) {
...
function createMainWindow() {
  ...
  const iconPaths = [
    ...
    path.join(__dirname, '..', 'resources', 'icons', 'app-icon.svg'),
    path.join(__dirname, '..', 'resources', 'icons', 'app-icon-256.png'),
  ...
  // Check for update completion flag
const updateFlagPath = path.join(app.getPath('userData'), 'updated.json');
let justUpdated = false;

if (fs.existsSync(updateFlagPath)) {
  try {
    const data = JSON.parse(fs.readFileSync(updateFlagPath, 'utf8'));
    if (data.justUpdated) {
      justUpdated = true;
      fs.unlinkSync(updateFlagPath); // clear it
    }
  } catch (e) {
    console.error('Error reading updated flag:', e);
  }
}
  ...
  // Enable screen sharing permissions for main window
  ...
  const contentView = new BrowserView({
    webPreferences: {
      ...
      contextIsolation: true,
      preload: path.join(__dirname, 'preload.js'),
      // Enable screen sharing and media access
      webSecurity: true,
  ...
  try {
    ...
    // Enable media permissions for the session
    contentView.webContents.session.setPermissionCheckHandler((webContents, permission) => {
      const allowedPermissions = [
        'media', 
        'display-capture',
        'mediaKeySystem',
    ...
    if (justUpdated) {
      contentView.webContents.loadFile(APP_UPDATE_COMPLETE_HTML).catch(console.error);
    } else {
      contentView.webContents.loadURL(APP_LOGIN_URL, { 
      userAgent: 'Chrome',
      // Resource optimization options
      httpReferrer: APP_LOGIN_URL,
      // Enable necessary features for screen sharing
      enableBlinkFeatures: 'GetDisplayMedia,MediaDevices',
    ...
    // Throttle background tabs to save CPU/GPU but allow media
  ...
  contentView.webContents.session.setPermissionRequestHandler((webContents, permission, callback) => {
    if (permission === 'display-capture' || permission === 'media' || permission === 'mediaKeySystem' || permission === 'geolocation' || permission === 'notifications' || permission === 'fullscreen' || permission === 'clipboard-read' || permission === 'clipboard-write') {
      return callback(true);
    }
    callback(false);
  });
  ...
  ipcMain.on('maximize-window', () => {
    ...
    if (mainWindow.isMaximized()) {
      ...
      // The 'maximize' and 'unmaximize' events will handle notifying the renderer
    } catch (error) {
  ...
  // Maximize/unmaximize event listeners consolidated above
...
function createTray() {
  ...
  // Load Turkish translations for tray/menu
  const trStrings = JSON.parse(fs.readFileSync(path.join(__dirname, 'locales', 'tr.json'), 'utf8'));
  // Create context menu
  const contextMenu = Menu.buildFromTemplate([
    {
      label: trStrings.open,
      click: () => {
        if (mainWindow) {
          mainWindow.show();
          mainWindow.focus();
    ...
    { type: 'separator' },
    {
      label: trStrings.create_server,
      click: () => {
        shell.openExternal('https://gangio.pro/login');
      }
    },
    {
      label: trStrings.join_server,
      click: () => {
        shell.openExternal('https://gangio.pro/login');
      }
    },
    {
      label: trStrings.discover_servers,
      click: () => {
        shell.openExternal('https://gangio.pro/discover');
      }
    },
    {
      label: trStrings.homepage,
      click: () => {
        shell.openExternal('https://gangio.pro');
      }
    },
    {
      label: trStrings.blog,
      click: () => {
        shell.openExternal('https://gangio.pro/blog');
      }
    },
    { type: 'separator' },
    {
      label: trStrings.preferences,
      submenu: [
        {
          label: trStrings.start_minimized,
          type: 'checkbox',
        ...
        {
          label: trStrings.minimize_to_tray,
          type: 'checkbox',
        ...
        {
          label: trStrings.launch_on_startup,
          type: 'checkbox',
    ...
    {
      label: trStrings.check_for_updates,
      click: () => {
        checkForUpdates();
      }
    },
...
function configureAutoUpdater() {
  ...
  autoUpdater.on('update-downloaded', (info) => {
    ...
    notification.on('action', () => {
      const updateFlagPath = path.join(app.getPath('userData'), 'updated.json');
      fs.writeFileSync(updateFlagPath, JSON.stringify({ justUpdated: true }));
...
app.whenReady().then(() => {
  // Set app icon for Windows notifications
  if (process.platform === 'win32') {
    app.setAppUserModelId(APP_ORGANIZATION + '.' + APP_NAME.replace(' ', ''));
  }
  
  // Enable screen capture permissions
  app.commandLine.appendSwitch('enable-features', 'MediaStreamAPI,GetDisplayMedia');
  ...
  // --- Start of globally guarded IPC Handlers --- 
  if (!appReadyIpcHandlersRegistered) {
    console.log('Registering app-ready IPC handlers...');

    // Set up IPC handlers for updates (individual guard no longer needed here)
    // if (!ipcMain.listenerCount('check-for-updates')) { // Removed individual guard
  ...
  // } // Removed individual guard
  
  // if (!ipcMain.listenerCount('install-update')) { // Removed individual guard
    ipcMain.on('install-update', () => {
      autoUpdater.quitAndInstall(false, true);
    });
  // The 'if (!appReadyIpcHandlersRegistered)' block continues...
  ...
  // Maximize window handler moved to line ~516
  ...
  // Removed duplicate 'is-window-maximized' handler from here. The primary one is later in the file.
  ...
  // Window maximize state handler (individual guard no longer needed here)
  // if (!ipcMain.listenerCount('is-window-maximized')) { // Removed individual guard
  ...
  // } else { 
  //   // console.warn("'is-window-maximized' handler already registered, skipping re-registration.");
  // } // Removed individual guard
  ...
  ipcMain.handle('set-tray-preference', (event, key, value) => {
    ...
    // All app-ready IPC handlers are now defined above this point.
    appReadyIpcHandlersRegistered = true;
    console.log('App-ready IPC handlers registration complete.');
  } else { // This else corresponds to if (!appReadyIpcHandlersRegistered)
    console.warn('App-ready IPC handlers already registered. Skipping re-registration.');
  }
  // --- End of globally guarded IPC Handlers ---
